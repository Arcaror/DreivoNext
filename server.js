...
const hostname = 'legrandarca.ddns.net'
const port = 3000
// when using middleware `hostname` and `port` must be provided below
const app = next({ dev, hostname, port })
const handle = app.getRequestHandler()
// const httpsOptions = {
//     key: fs.readFileSync("./certificates/certificat.key"),
//     cert: fs.readFileSync("./certificates/certificat.crt"),
//   };
var dict = new Map()
    ...
    createServer( async (req, res) => {
        try {
            ...
            if (pathname === '/a') {
                ...
                // if (dict.get(ip).count > (dict.get(ip).duration + 5) * 1000) {
                //     setTimeout(async () => {
                //         dict.set(ip, {
